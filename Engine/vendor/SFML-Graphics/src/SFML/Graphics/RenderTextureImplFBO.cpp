...
m_multisample       (false),
m_stencil           (false),
m_frameBuffer       (std::nullopt),
m_multisampleFrameBuffer(std::nullopt)
{
...
bool RenderTextureImplFBO::createFrameBuffer()
{
    ...
    if (m_multisample)
    {
...

        {
            ...
            // Insert the FBO into our map
            m_multisampleFrameBuffer= static_cast<unsigned int>(multisampleFrameBuffer);
        }
    }
...
void RenderTextureImplFBO::updateTexture(unsigned int)
{
    ...
    if (m_multisample && m_width && m_height && activate(true))
    {
        ...
        if (m_frameBuffer.has_value() && m_multisampleFrameBuffer.has_value())
        {
            // Set up the blit target (draw framebuffer) and blit (from the read framebuffer, our multisample FBO)
            glCheck(GLEXT_glBindFramebuffer(GLEXT_GL_DRAW_FRAMEBUFFER, m_frameBuffer.value()));
            glCheck(GLEXT_glBlitFramebuffer(0, 0, m_width, m_height, 0, 0, m_width, m_height, GL_COLOR_BUFFER_BIT, GL_NEAREST));
            glCheck(GLEXT_glBindFramebuffer(GLEXT_GL_DRAW_FRAMEBUFFER, m_multisampleFrameBuffer.value()));
        }
    }
